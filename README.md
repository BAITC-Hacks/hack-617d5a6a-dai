...
Шаг `cp` обязателен: без `.env.development` в браузере пустая страница, потому что клиент не знает адрес API. `make web` делает только `npm ci` и `npm run dev`, поэтому при первом запуске файл нужно скопировать вручную.
