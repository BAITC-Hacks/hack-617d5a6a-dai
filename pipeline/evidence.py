...
def cluster_hypothesis(c) -> str:
    ...
    tail = f"{plural(c.n_boundary, 'узел', 'узла', 'узлов')} на границе выгрузки"
    # гипотеза о назначении группы (для проверки, не вывод о клиентах)
    if n == 1:
        purpose = "гипотезы нет: изолированный клиент"
    elif c.n_seed >= 2:
        purpose = "гипотеза: общая сеть нескольких исходных клиентов, сбор и перераспределение между ними"
    elif c.n_seed == 1 and n and c.n_boundary / n >= 0.3:
        purpose = "гипотеза: ветвь одного исходного клиента, конец потока за границей выгрузки"
    elif c.n_seed == 1:
        purpose = "гипотеза: ветвь распределения одного исходного клиента"
    else:
        purpose = "гипотеза: группа получателей без исходного клиента внутри, вход из других групп"
    text = f"{head}; {mid}; {tail}; {purpose}"
    return text if len(text) <= 200 else f"{head}; {tail}; {purpose}"[:200]
