...
(function () {
  ...
  function create(el, cb) {
    ...
    let layoutTimer = null, destroyed = false;
    const duration = (ms) => reduced ? 0 : ms;
    const root = d3.select(el).style('position', 'absolute').style('inset', '0').style('background', BG).style('background-image', 'radial-gradient(ellipse at 50% 45%, #252334 0%, transparent 65%)').style('overflow', 'hidden');
    const svg = root.append('svg').attr('width', '100%').attr('height', '100%').style('display', 'block').style('cursor', 'grab').attr('aria-label', 'Интерактивный граф переводов').attr('role', 'group');
    ...
    defs.append('marker').attr('id', uid + '-arr').attr('viewBox', '0 -4 8 8').attr('refX', 7).attr('refY', 0).attr('markerWidth', 7).attr('markerHeight', 7).attr('markerUnits', 'userSpaceOnUse').attr('orient', 'auto')
    ...
    let k = 1;
    const zoom = d3.zoom().scaleExtent([0.15, 6]).on('zoom', (e) => { g.attr('transform', e.transform); const nk = e.transform.k; k = nk; labels(0); });
    ...
    function place(animate) {
      ...
      if (V.mode === 'local' && V.layout === 'layers') {
        ...
        layoutTimer = d3.timer(() => {
          ...
          draw(); if (p >= 1) { layoutTimer.stop(); layoutTimer = null; }
        });
        return;
      }
      ...
      if (fresh.length > nodes.length * 0.5) { sim.alpha(1); for (let i = 0; i < (reduced ? 240 : 55); i++) sim.tick(); draw(); if (!reduced) sim.alpha(0.45).restart(); }
      else if (reduced) { sim.alpha(0.5); for (let i = 0; i < 180; i++) sim.tick(); draw(); } else sim.alpha(animate ? 0.5 : 0.3).restart();
    }
    ...
    function render(animate) {
      ...
      ).attr('stroke', '#8a8a90').attr('stroke-width', ew).attr('marker-end', V.flow === 'arrows' ? 'url(#' + uid + '-arr)' : null);
      ...
      nodeSel.attr('tabindex', 0).attr('role', 'button').attr('aria-label', (o) => o.id + ', ' + DAI.role(o.n.role).title)
        .on('keydown', (e, o) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); cb.onSelect && cb.onSelect(o.id); } })
        .on('focus', (e, o) => hover(o.id)).on('blur', () => hover(null));
    ...
    function hulls(animate) {
      ...
      hullSel.select('path').attr('fill', (d) => (V.colorBy === 'cluster' ? CL[d[0] % CL.length] : '#ffffff')).attr('fill-opacity', 0.025).attr('stroke', (d) => (V.colorBy === 'cluster' ? CL[d[0] % CL.length] : '#ffffff')).attr('stroke-opacity', 0.08);
      hullSel.select('text').text((d) => { const p = passports.get(d[0]); return 'Кластер ' + d[0] + ' · ' + d[1].length + (p ? ' из ' + DAI.groups(p.n_nodes) : '') + ' узлов'; });
    }
    ...
    function opac(dur) {
      ...
      linkSel.transition().duration(dur).attr('stroke-opacity', (l) => {
        ...
        return 0.32;
      }).attr('stroke', (l) => (H && (l.source.id === hoverId || l.target.id === hoverId) ? color(l.source) : '#777786'));
      hullSel.transition().duration(dur).attr('opacity', H ? 0.35 : 1);
    }
    function labels(dur) {
      dur = duration(dur);
      const H = hoverId ? nb(hoverId) : null;
      // Compensate SVG camera scaling once labels reach their screen-space cap.
      const fontSize = (o) => Math.min(o.id === V.focus ? 12 : 10, (o.id === V.focus ? 14 : 12) / k);
      textSel.attr('font-size', fontSize).attr('stroke-width', Math.min(3, 3 / k))
        .attr('y', (o) => o.y + R(o) + Math.min(12, 14 / k));
      hullSel.select('text').attr('font-size', Math.min(12, 12 / k));
      const boxes = [], shown = new Set();
      const ranked = vis.nodes.slice().sort((a, b) =>
      ...
      for (const o of ranked) {
        if (o.ghost) continue;
        const important = o.id === hoverId || o.id === V.focus;
        const eligible = important || (H ? H.has(o.id) : top.has(o.id) || k >= V.labelZoom || (V.mode === 'local' && vis.nodes.length <= 30));
        if (!eligible) continue;
        const font = fontSize(o), padding = 6 / k;
        const width = String(DAI.gidParts(o.id).mid).length * font * 0.65 + padding * 2;
        const baseline = o.y + R(o) + Math.min(12, 14 / k);
        const box = { x: o.x - width / 2, y: baseline - font - padding / 2, w: width, h: font + padding };
        if (important || !boxes.some(b => box.x < b.x + b.w && box.x + box.w > b.x && box.y < b.y + b.h && box.y + box.h > b.y)) {
          shown.add(o.id); boxes.push(box);
        }
      }
      textSel.interrupt().transition().duration(dur).attr('opacity', o => shown.has(o.id) ? 1 : 0);
    }
    function flows() {
    ...
    function placeTip(x, y) { const box = tip.node().getBoundingClientRect(); tip.style('left', Math.max(8, Math.min(x + 14, el.clientWidth - box.width - 8)) + 'px').style('top', Math.max(8, Math.min(y + 14, el.clientHeight - box.height - 8)) + 'px'); }
    ...
    const resize = new ResizeObserver(() => { if (!first && !destroyed) fit(0); });
    resize.observe(el);
    svg.on('click', () => cb.onBackground && cb.onBackground());
    return { setData, setView, flyTo, hover: (id) => hover(id), fit: () => fit(600), counts: () => ({ n: vis.nodes.filter((o) => !o.ghost).length, e: vis.edges.filter((l) => !l.ghost).length }), zoomBy: (factor) => svg.transition().duration(duration(250)).call(zoom.scaleBy, factor), destroy: () => { destroyed = true; resize.disconnect(); if (camT) camT.stop(); if (layoutTimer) layoutTimer.stop(); sim.stop(); root.selectAll('*').interrupt(); root.selectAll('*').remove(); } };
  }
