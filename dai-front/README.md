...
Экран «Узел» (`/nodes/$gid`, стартовая страница открывает первый узел очереди) работает на реальных данных бэкенда. Макет — Claude Design, решения — в [DESIGN.md](DESIGN.md).
...
| Схема сети | окружение узла 1–4 хопа (слои / force) и общий вид с кластерами-островами; направление денег, роли, seed, граница выгрузки; настройки, фильтры, таймлапс по переводам загруженных карточек | готово |
...
src/routes/nodes.$gid.tsx экран «Узел»: шапка, очередь, схема сети, карточка
