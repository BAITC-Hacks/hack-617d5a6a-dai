...
function NodeCardColumn() {
  ...
  // Состояние живёт до закрытия карточки (Esc): смена узла его не сбрасывает.
  ...
  return (
    <aside className={cn('min-h-0 overflow-auto border-l bg-background transition-[width] duration-200', open ? 'w-[360px] min-[1440px]:w-[440px]' : 'w-12')}>
      {/* Стрелка видна при прокрутке, место под себя не занимает */}
      <div className={cn('sticky top-0 z-10 flex h-0', open ? 'justify-end pr-2.5' : 'justify-center')}>
        <Button
          variant="ghost"
          size="icon"
